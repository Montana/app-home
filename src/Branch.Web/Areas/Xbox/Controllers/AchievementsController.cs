...
namespace Branch.Web.Areas.XboxLive.Controllers
{
		...
		: ControllerBase
	{
		...
		public async Task<IActionResult> Index(string gamertag, uint page = 0)
		{
			...
			var durangoTitleHistoryTask = TitleHistoryService.GetDurangoTitleHistory(playerXuid, (AchievementsPerPage * (int)page), (AchievementsPerPage * (int)page) + AchievementsPerPage);
			var legacyTitleHistoryTask = TitleHistoryService.GetLegacyTitleHistory(playerXuid, (AchievementsPerPage * (int)page), (AchievementsPerPage * (int)page) + AchievementsPerPage);
			var profileSummaryTask = UserService.GetProfileSummary(playerXuid);
			var profilePreferredColorTask = UserService.GetProfileColour(profileSettings.Settings.First(s => s.Id == "PreferredColor").Value);
		...
		public async Task<IActionResult> GetXboxOneAchievementDetails(string gamertag, string titleId, string slug)
		{
			...
			var profileSummaryTask = UserService.GetProfileSummary(profileSettings.Xuid);
			var profilePreferredColorTask = UserService.GetProfileColour(profileSettings.Settings.First(s => s.Id == "PreferredColor").Value);
